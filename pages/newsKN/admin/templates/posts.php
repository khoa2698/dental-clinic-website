...
if ($user) {
    ...
    if ($ac != '') {
        ...
        // Trang chỉnh sửa bài viết
            ...
            {
                $data_post = $db->fetch_assoc($sql_check_id_cate, 0);
                ...
                if ($data_post['author_id'] == $data_user['id_acc'] || $data_user['position'] == '1')
                {
                    ...
                    WHERE id_post = '$id';
                    ";
                    $data_post = $db->fetch_assoc($qr_get_data_post, 0);
                    echo
                    '
                    ...
                    $qr_get_cate_post_1 = "
                    SELECT label, id_cate FROM categories
                    WHERE type = '1';
                    ";
                    if ($db->num_rows($qr_get_cate_post_1)) {
                        if ($data_post['cate_1_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->fetch_assoc($qr_get_cate_post_1, 1) as $key => $data_cate_1) {
                    ...
                    $qr_get_cate_post_2 = "
                    SELECT label, id_cate FROM categories
                    WHERE type = '2' AND parent_id = '$data_post[cate_1_id]';
                    ";
                    if ($db->num_rows($qr_get_cate_post_2)) {
                        if ($data_post['cate_2_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->fetch_assoc($qr_get_cate_post_2, 1) as $key => $data_cate_2) {
                    ...
                    $qr_get_cate_post_3 = "SELECT label, id_cate FROM categories WHERE type = '3' AND parent_id = '$data_post[cate_2_id]';";
                    if ($db->num_rows($qr_get_cate_post_3)) {
                        if ($data_post['cate_3_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->fetch_assoc($qr_get_cate_post_3, 1) as $key => $data_cate_3) {
    ...
    else
    {
        ...
        if ($data_user['position'] == 1) {
            $qr_get_list_post = " SELECT * FROM posts ORDER BY id_post DESC;";
        } 

        // Nếu là tác giả thì chỉ lấy những bài thuộc sở hữu
        else {
            $qr_get_list_post = "SELECT * FROM posts WHERE author_id = '$data_user[id_acc]' ORDER BY id_post DESC;";
        }
        // nếu có bài viết
        if ($db->num_rows($qr_get_list_post)) {
            // Lấy số trang
            if (isset($_GET['page'])) {
            ...
            $limit = 10; // giới hạn 10 bài viết trong 1 trang
            $total_page = ceil($db->num_rows($qr_get_list_post)/ $limit);
            $start = ($current_page - 1) * $limit; // trang hiện tại bắt đầu từ tin thứ mấy
            ...
            // Nếu là admin thì lấy toàn bộ bài viết
            if ($data_user['position'] == '1') {
                $qr_get_list_post_limit = "
                SELECT * FROM posts ORDER BY id_post DESC 
                LIMIT $start, $limit;
                ";
            }
            // Nếu là tác giả thì lấy những bài sở hữu
            else {
                $qr_get_list_post_limit = "
                SELECT * FROM posts 
                WHERE author_id = '$data_user[id_acc]'
            ...
            foreach($db->fetch_assoc($qr_get_list_post_limit, 1) as $key => $data_post) {
                // Trạng thái bài viết
                ...
                // chuyên mục bài viết
                $cate_post = '';
                $qr_check_id_cate_1 = "SELECT label, id_cate FROM categories WHERE id_cate = '$data_post[cate_1_id]' AND type = '1';";
                if ($db->num_rows($qr_check_id_cate_1)) {
                    $data_cate_1 = $db->fetch_assoc($qr_check_id_cate_1, 0);
                    $cate_post .= $data_cate_1['label'];
                } else {
                    $cate_post .= '<span class="text-danger">Lỗi</span>';
                }
        
                $qr_check_id_cate_2 = "SELECT label, id_cate FROM categories WHERE id_cate = '$data_post[cate_2_id]' AND type = '2';";
                if ($db->num_rows($qr_check_id_cate_2)) {
                    $data_cate_2 = $db->fetch_assoc($qr_check_id_cate_2, 0);
                    $cate_post .= ', ' . $data_cate_2['label'];
                } else {
                    $cate_post .= ', <span class="text-danger">Lỗi</span>';
                }
        
                $qr_check_id_cate_3 = "SELECT label, id_cate FROM categories WHERE id_cate = '$data_post[cate_3_id]' AND type = '3';";
                if ($db->num_rows($qr_check_id_cate_3)) {
                    $data_cate_3 = $db->fetch_assoc($qr_check_id_cate_3, 0);
                ...
                // Tác giả bài viết
                $qr_get_author = "SELECT display_name FROM accounts WHERE id_acc = '$data_post[author_id]';";
                if ($db->num_rows($qr_get_author)) {
                    $data_author = $db->fetch_assoc($qr_get_author, 0);

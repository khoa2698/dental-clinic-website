...
if ($user)
{
    ...
    else if ($data_user['position'] == 1)
    {
        ...
        if ($db->num_rows($sql_get_stt_web)) {
            $data_web = $db->fetch_assoc($sql_get_stt_web, 0);
        ...
        echo '
                    <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-floppy-save"></span> Lưu</button><br><br>
        ...
        if ($db->num_rows($sqlGetInfoWeb)) {
            $data_web = $db->fetch_assoc($sqlGetInfoWeb, 0);
        }
                ...
                <div class="panel-body">
                    <form method="POST" id="formInfoWeb" onsubmit="return false;">
                        <div class="form-group">
                            <label for="title_web">Tiều đề website</label>
                            <input type="text" class="form-control" value="' . $data_web['title'] . '" id="title_web">
                        </div>
                        <div class="form-group">
                            <label for="descr_web">Mô tả website</label>
                            <textarea class="form-control" id="descr_web">' . $data_web['descr'] . '</textarea>
                        </div>
                        <div class="form-group">
                            <label for="keywords_web">Từ khoá website</label>
                            <input type="text" class="form-control" value="' . $data_web['keywords'] . '" id="keywords_web">
                        </div>
                        <button class="btn btn-primary" type="submit"><span class="glyphicon glyphicon-floppy-save"></span> Lưu</button><br><br>
                        <div class="alert alert-danger hidden"></div>
                    </form>
                </div>
            </div>

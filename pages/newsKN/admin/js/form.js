$_DOMAIN = 'http://localhost/DentalClinic/pages/newsKN/admin/';

// Sign in
$('#formSignin button').on('click', ()=> {
    $(this).html('Đang tải');
...
$('#formEditCate input[type="radio"]').on('click', () => {
    ...
    if ($('#formEditCate .type-edit-cate-1:checked').prop('checked') == true) {
        ...
        $.ajax({
            ...
            error: ()=> {
                $('.parent-edit-cate').removeClass('hidden');
                $('.parent-edit-cate').html('Đã có lỗi xảy ra, thử lại sau');
            },
        })
    }
...
function preUpImg() {
    ...
    if (img_up != '')
    {
        ...
        {
            $('#formUpImg .box-pre-img').append('<img src="' + URL.createObjectURL(img_up_arr[i]) + '" style="border: 1px solid #ddd; width: 200px; margin-right: 5px; margin-bottom: 5px;"/>');
        }
...
$('#del_img_list').on('click', function() {
    $confirm = confirm('Bạn có chắc chắn muốn xóa hình ảnh đã chọn');
    if ($confirm == true) {
        $id_img = [];
    ...
    if ($title_add_post == '' || $slug_add_post == '') {
        ...
        $.ajax({
            ...
            data: {
                ...
                // $('#formAddPost .alert').removeClass('hidden');
                $('#formAddPost .alert').html(result);
            }, error: function() {
                $('#formAddPost .alert').removeClass('hidden');
                $('#formAddPost .alert').html('Đã có lỗi xảy ra, vui lòng thử lại');
            }
        });
    }
});
...
$('#cate_post_1').on('change', function() {
    ...
    $.ajax({
        ...
        data: {
            ...
            $('#cate_post_2').html(result);

            // Tải chuyên mục nhỏ sau khi đã tải xong chuyên mục vừa
...
$('#formAddAcc button').on('click', function() {
    $user_add_acc = $('#user_add_acc').val();
    $pw_add_acc = $('#pw_add_acc').val();
    $repw_add_acc = $('#repw_add_acc').val();
    if ($('#formAddAcc .position_add_acc_1:checked').prop("checked") == true) {
        $position_add_acc = $('#formAddAcc .position_add_acc_1').val();
    }
    if ($('#formAddAcc .position_add_acc_2:checked').prop("checked") == true) {
        $position_add_acc = $('#formAddAcc .position_add_acc_2').val();
    }
    if ($('#formAddAcc .status_add_acc_1:checked').prop("checked") == true) {
        $status_add_acc = $('#formAddAcc .status_add_acc_1').val();
    }
    if ($('#formAddAcc .status_add_acc_2:checked').prop("checked") == true) {
        $status_add_acc = $('#formAddAcc .status_add_acc_2').val();
    }
    ...
    if ($user_add_acc == '' || $pw_add_acc == '' || $repw_add_acc == '') {
        ...
        $.ajax({
            ...
            data : {
                ...
                position_add_acc : $position_add_acc,
                status_add_acc : $status_add_acc
            }, success: function(result) {
                $('#formAddAcc .alert').html(result);
            }, error : function() {
                $('#formAddAcc .alert').removeClass('hidden');
...
// Khóa tài khoản chỉ định trong trang danh sách acc
...
$('#unlock_acc_list').on('click', function() {
    $confirm = confirm('Bạn có chắc chắn muốn mở khoá tài khoản đã chọn không?');
    if ($confirm == true) {
        $id_acc = [];
        $('#list_acc input[type="checkbox"]:checkbox:checked').each(function(i) {
            $id_acc[i] = $(this).val();
...
// Xem trước ảnh avatar update
// Ẩn chosse file mặc định
fileElem_UpAvt = $('#formUpAvt #img_avt');
fileSelect_UpAvt = $("#formUpAvt #fileSelect");

fileSelect_UpAvt.on('click', function() {
    if (fileElem_UpAvt) {
        fileElem_UpAvt.click();
    }
});
...
function preUpAvt() {
    img_avt = $('#img_avt').val();
    console.log(img_avt);
    img_up_arr = $('#img_avt').get(0).files;
    if (img_avt != '') {
        $('#formUpAvt .box-pre-img').removeClass('hidden');
        // Nếu tồn tại một thẻ <img> rồi thì sẽ đổi giá trị của thuộc tính 'src'
        if($('#formUpAvt .box-pre-img img').length){
            $('#formUpAvt .box-pre-img img').attr('src', URL.createObjectURL(img_up_arr[0]));
        } 
        // Nếu chưa tồn tại thì thêm thẻ <img> mới
        else {
            $('#formUpAvt .box-pre-img').append('<img src="'+ URL.createObjectURL(img_up_arr[0]) +'" style="border:1px solid #dđ; width: 200px; margin-right: 5px; margin-bottom: 5px;"/>');
        }
    }
    // Nếu chưa chọn ảnh
    else
...
$('#formUpAvt').submit(function(e) {
    ...
    if (img_avt) {
        ...
        if (size_img_avt > 5242880) {
            ...
            $('#img_avt .alert-danger').removeClass('hidden');
            $('#img_avt .alert-danger').html('Tệp đã chọn có dung lượng lớn hơn mức cho phép.');
        }
        // Lỗi về định dạng ảnh
...
$('#formUpdateInfo button').on('click', function() {
    ...
    if($dn_update && $email_update) {
        ...
        $('#formUpdateInfo button').html('Lưu thay đổi');
        $('#formUpdateInfo .alert').removeClass('hidden');
        $('#formUpdateInfo .alert').html('Vui lòng điền đầy đủ thông tin bắt buộc.' + '<br>' + '(*): bắt buộc');
    }
});

// Đổi mật khẩu
$('#formChangePw button').on('click', () => {
    $('#formChangePw button').html('Đang tải...');
    $old_pw_change = $('#formChangePw #old_pw_change').val();
    $new_pw_change = $('#formChangePw #new_pw_change').val();
    $re_new_pw_change = $('#formChangePw #re_new_pw_change').val();

...
if ($db->num_rows($sql_get_data_post)) {
    $data_post = $db->fetch_assoc($sql_get_data_post, 0);
} else {
    // Nếu không tồn tại 
...
<div class="container" style="margin-top: 30px;">
    <div class="row" style="flex-direction: column; width:min-content; margin-left: 0; margin-right: 0;">
        <div>
            <h3><?php echo $data_post['title']; ?></h3>
        </div>
        <div>
            <h6>Ngày đăng: <?php echo $data_post['date_posted']; ?></h6>
        </div>
        <div class="body-post" style="text-align: justify;">
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                if ($id_cate) {
                    ...
                    if ($db->num_rows($sql_get_data_cate)) {
                        $data_cate = $db->fetch_assoc($sql_get_data_cate, 0);
    ...
    <div class="row" style="margin-bottom: 30px;">
        ...
        $sql_get_invole_post = "SELECT DISTINCT * FROM posts WHERE (cate_1_id = '$data_post[cate_1_id]' OR cate_2_id = '$data_post[cate_2_id]' OR cate_3_id = '$data_post[cate_3_id]') AND status = '1' AND id_post != '$id' LIMIT 4;";
        // Nếu tồn tại các bài viết liên quan
        if ($db->num_rows($sql_get_invole_post)) {
            // In danh sách bài viết liên quan
            foreach ($db->fetch_assoc($sql_get_invole_post, 1) as $data_post) {
                echo '
                    <div class="col-md-3">
                        <div class="thumbnail">
